@extends('layouts.app')
@section('title','Edit Model/Type')
@section('content')
<div class="max-w-xl mx-auto">
  <div class="bg-gray-800/60 rounded-2xl p-8 mt-10">
    <h2 class="text-2xl font-bold mb-6 text-white">Edit Model/Type</h2>
    <form action="{{ route('model-types.update', $modelType) }}" method="POST" class="space-y-5">
      @csrf @method('PUT')
      <div>
        <label class="block mb-1 text-gray-300">Name</label>
        <input name="name" value="{{ old('name', $modelType->name) }}"
      ...
      <div>
        ...
        <input name="minimum_stock" type="number" min="0" value="{{ old('minimum_stock', $modelType->minimum_stock) }}"
               class="w-full px-4 py-2 rounded-lg bg-gray-900 text-white border border-gray-700" required />
